/***********************************************************************
 *                                                                     *
 * $Id: hpgslabel.c 489 2009-02-11 14:10:30Z softadm $
 *                                                                     *
                                      ...
                                      const hpgs_point *space_vec)
{
  ...
  if (!inbuf)
    {
      hpgs_set_error(hpgs_i18n("unable to allocate %d bytes of temporary string data."),
                     str_len);
      goto cleanup;
    }
  ...
  if (len == (size_t)(-1))
    {
      hpgs_set_error(hpgs_i18n("unable to convert string to encoding %s: %s."),
