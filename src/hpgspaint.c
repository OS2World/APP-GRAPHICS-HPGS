/***********************************************************************
 *                                                                     *
 * $Id: hpgspaint.c 489 2009-02-11 14:10:30Z softadm $
 *                                                                     *
...
static  int pdv_stroke      (hpgs_device *_this)
{
  ...
  hpgs_point d;
  hpgs_bbox_distance(&d,&pdv->page_bb,&pdv->path->bb);
  
  if (d.x < pdv->gstate->linewidth && d.y < pdv->gstate->linewidth) 
    {
...
static  int pdv_fill        (hpgs_device *_this, hpgs_bool winding)
{
  ...
  if (d.x > 0.0 || d.y > 0.0) 
    ...
    ret = hpgs_paint_device_fill(pdv,pdv->path,winding,HPGS_FALSE);
...
static  int pdv_setpatcol(hpgs_device *_this, const hpgs_color *rgb)
{
  ...
  return hpgs_image_setpatcol(pdv->image,&pdv->patcol);
}
