/***********************************************************************
 *                                                                     *
 * $Id: hpgspath.c 489 2009-02-11 14:10:30Z softadm $
 *                                                                     *
...
static int shade(hpgs_reader *reader, double level, hpgs_bool winding)
{
  ...
  if  (alpha != 1.0)
    {
      ...
      if (hpgs_device_setrgb_all(reader->device,&rgb))
        return -1;
    }
  ...
  return hpgs_device_setrgb_all(reader->device,
...
int hpgs_reader_do_CI (hpgs_reader *reader)
{
  ...
  if (reader->polygon_mode)
    ...
    // This is necessary here in order to enforce a call to hpgs_moveto()
    // inside arc_to_bezier() lateron. This fixes the rpoblem with two
    // conecutive CI statements.
    reader->polygon_open = 0;
  }
