/***********************************************************************
 *                                                                     *
 * $Id: hpgsfont.c 489 2009-02-11 14:10:30Z softadm $
 *                                                                     *
...
static void hpgs_scan_font_dir(const char *path, int *npaths, int *nerrors);

static int hpgs_scan_font_dir_internal(const char *path, int *npaths, int *nerrors)
{
  // go through font directory.
  DIR *d;
  struct dirent *data;

#ifdef DEBUG_FONT_SEARCH
                fprintf(stderr,"Scanning font directory <%s>...\n",
                        path);
#endif
      ...
      if (data->d_type == DT_UNKNOWN)
        {
          ...
          if (stat(fn,&statbuf))
            {
              hpgs_log("Warning: Cannot stat file %s: %s\n",fn,strerror(errno));
              goto cleanup_continue;
            }

          data->d_type = IFTODT(statbuf.st_mode);
        }

      if (data->d_type == DT_DIR)
        {
          hpgs_scan_font_dir(fn,npaths,nerrors);
          goto cleanup_continue;
        }
      else if (data->d_type == DT_REG)
        {
          int l = strlen(data->d_name);
          if (l<4) goto cleanup_continue;

          if (strcmp(data->d_name+l-4,".ttf") &&
              strcmp(data->d_name+l-4,".TTF")   ) goto cleanup_continue;

          is = hpgs_new_file_istream(fn);
          if (!is) goto cleanup_continue;
          ...
          if (read_header(&header,is) == 0)
            {
              ...
              if (name) {
                ...
                fprintf(stderr,"Found font <%s> in file <%s>.\n",
                        name,fn);
#endif
                // avoid double-freeing the filename.
                fn = 0;
              }
            }
...
#ifdef DEBUG_FONT_SEARCH
  fprintf(stderr,"Finished scanning font directory <%s>.\n",
          path);
#endif
...
{
  int ret = 
    hpgs_scan_font_dir_internal(path,npaths,nerrors);
...
static int hpgs_scan_font_dirs()
{
  ...
  if (!font_path || strlen(font_path) <= 0)
    font_path = "/usr/X11R6/lib/X11/fonts:/usr/share/fonts:/usr/local/share/fonts";

  char *font_path_dup = hpgs_alloca(strlen(font_path)+1);
      ...
      if (*fp == ':' || *fp == 0)
        {
          ...
          if (fp - last_font > 0)
            hpgs_scan_font_dir(last_font,&npaths,&nerrors);
